...
import nc_config
from ncDef import getAlllevel, setMembership
    ...
    for db_table in ("adjacency", "lr_node", "lr_grid", "lr_cluster"):
        ...
        if db_table == "adjacency":
            ...
            for lr_info in api_data:
                ...
                redis_client.hmset("lrid:%s:%s"%(lr_info["level_id"], lr_info["id"]), {"sysload": '0', "ip": lr_info["ip"], "port": lr_info["port"], "lr_type": lr_info["lr_type"], "last_subtime": cur_time, "active": '1'})
                set_level_res = setMembership(lr_info["level_id"], lr_info["p_node_level"])
                if set_level_res: logger.info("New levels add success.")
